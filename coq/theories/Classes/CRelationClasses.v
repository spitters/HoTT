...
Require Export Init Logic Datatypes.
...
Section Defs.
  ...
  (** These are convertible. but we don't have = yet 
...
*)
  ...
  (** Leibniz equality. This is done in Overture.
  Section Leibniz. 
    Global Instance eq_Reflexive : Reflexive (@identity A) := @identity_refl A.
    Global Instance eq_Symmetric : Symmetric (@paths A) := @paths_sym A.
    Global Instance eq_Transitive : Transitive (@paths A) := @paths_trans A.
...
Ltac simpl_crelation :=
  unfold flip, arrow ; try reduce 
(* ; program_simpl Bas:probably the use of subst.*).
